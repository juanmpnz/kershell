...
Both runtimes use an enforced, self-hosted policy from
`packages/config/security-headers.ts`. Scripts, styles, images, fonts,
connections, forms and workers default to local resources. Objects and framing
are blocked. Production also upgrades insecure requests and enables HSTS.
